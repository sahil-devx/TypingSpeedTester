...
let keylistener;
keylistener = function(e) {  // Desktop key handling
    e.preventDefault();
    ...
    if (e.key.length > 1) return; // Skip non-character keys except Backspace
...
mobileInput.addEventListener("input", (e) => {
    ...
    if (char === currentSpan.textContent) {
        currentSpan.style.color = "green";
        const correctSound = new Audio(correctSoundsrc);
        correctSound.play();
    } else {
    ...
    currentIdx++;
    mobileInput.value = ""; // Clear for next character
});
...
startBtn.addEventListener("click", () => {
    ...
    focusMobileInput(); // Show mobile keyboard
...
function start() {
    ...
    clearInterval(stop);         //stopping the following code if already running
    stop = setInterval(() => {
    ...
    sp = setTimeout(()=>{                                                               //checking the correct no. of spans
        ...
        stopwatch.classList.add("deactive");      //adds stopwatch out-animation
...
// === MOBILE AUTO-FOCUS ON TAP ANYWHERE ===
testText.addEventListener("touchstart", focusMobileInput);
